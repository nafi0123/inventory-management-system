...
// কম্পোনেন্টের ভেতর ব্যবহারের জন্য স্কেলিটন রো
const TableSkeleton = () => (
  <>
    {[...Array(6)].map((_, i) => (
      <tr key={i} className="animate-pulse">
        ...
        <td className="px-8 py-5 text-right border-b border-gray-50">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden min-h-[460px] flex flex-col">
                  ...
                  <tr key={cat._id} className="hover:bg-gray-50/50 transition-all group">
                    ...
                    <td className="px-8 py-5">
                      <div className="flex flex-wrap gap-1.5">
                        ...
                        )) : <span className="text-gray-300 text-[10px] italic font-medium uppercase tracking-widest">No Extra Specs</span>}
                      </div>
                    </td>
        ...
        <div className="px-8 py-5 bg-gray-50/50 flex items-center justify-between border-t border-gray-100 mt-auto">
          ...
          <div className="flex gap-3">
            <button disabled={currentPage === 1 || initialLoading} onClick={() => setCurrentPage(prev => prev - 1)} className="p-2.5 bg-white border border-gray-200 rounded-xl shadow-sm disabled:opacity-20 transition-all hover:bg-gray-50"><ChevronLeft size={20} /></button>
            <button disabled={currentPage === totalPages || totalPages === 0 || initialLoading} onClick={() => setCurrentPage(prev => prev + 1)} className="p-2.5 bg-white border border-gray-200 rounded-xl shadow-sm disabled:opacity-20 transition-all hover:bg-gray-50"><ChevronRight size={20} /></button>
          </div>
        </div>
      </div>
              ...
              <div className="space-y-6">
                ...
                <div>
                  <div className="flex justify-between items-center mb-3">
                    ...
                    <button onClick={() => setExtraFields([...extraFields, ""])} className="text-[9px] bg-black text-[#f9db3d] px-4 py-2 rounded-xl font-black uppercase transition-transform active:scale-95">+ Add Field</button>
                  </div>
                ...
                <button onClick={handleSaveCategory} disabled={loading} className="w-full py-4 bg-[#f9db3d] hover:bg-[#e6c930] text-black font-black uppercase rounded-2xl shadow-xl transition-all active:scale-[0.98]">
